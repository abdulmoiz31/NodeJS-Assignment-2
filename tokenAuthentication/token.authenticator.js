...
function authenticateAdminToken(req, res, next) {
   ...
   function authenticateToken(req, res, next) {
    ...
    jwt.verify(token, String(process.env.TOKEN_SECRET), async function(err, user) {
    console.log("Err:", err)
    let valid = await checkTokenInDB(token, user)
     if (err || !valid || ((user.userType != userTypes.studentUser) && (user.userType != userTypes.teacherUser) && (user.userType != userTypes.adminUser))) return res.sendStatus(403)
